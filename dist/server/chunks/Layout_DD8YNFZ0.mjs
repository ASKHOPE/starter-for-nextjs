...
import 'piccolore';
import { h as addAttribute, x as renderHead, r as renderTemplate, q as renderComponent, y as renderSlot } from './server_CnwHzkPb.mjs';
...
const $$Layout = createComponent(($$result, $$props, $$slots) => {
  ...
  ].map((item) => renderTemplate`<a${addAttribute(item.href, "href")}${addAttribute(`flex items-center gap-4 px-5 py-4 rounded-[1.5rem] font-black text-sm transition-all group ${pathname === item.href || item.href !== "/" && pathname.startsWith(item.href) ? "bg-secondary text-white shadow-xl shadow-secondary/20" : "text-on-surface-variant hover:bg-secondary/5 hover:text-secondary"}`, "class")}> ${item.label} </a>`)} </nav> </div> </aside>  <div class="flex-1 flex flex-col min-h-screen min-w-0"> ${renderComponent($$result, "TopAppBar", TopAppBar, { "client:load": true, "client:component-hydration": "load", "client:component-path": "/Users/hosanna/Documents/GitHub/mychurchcallings/src/components/layout/TopAppBar", "client:component-export": "TopAppBar" })} <main class="flex-1 w-full max-w-6xl mx-auto px-4 sm:px-8 lg:px-12 py-8 md:py-12 pb-32 md:pb-12"> <div class="animate-in fade-in slide-in-from-bottom-6 duration-1000"> ${renderSlot($$result, $$slots["default"])} </div> </main> </div> </div>  <div class="md:hidden"> ${renderComponent($$result, "BottomNav", BottomNav, { "client:load": true, "pathname": pathname, "client:component-hydration": "load", "client:component-path": "/Users/hosanna/Documents/GitHub/mychurchcallings/src/components/layout/BottomNav", "client:component-export": "BottomNav" })} </div></body></html>`;
}, "/Users/hosanna/Documents/GitHub/mychurchcallings/src/layouts/Layout.astro", void 0);
